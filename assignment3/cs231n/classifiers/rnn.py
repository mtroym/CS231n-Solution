...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        - captions: Ground-truth captions; an integer array of shape (N, T) where
          each element is in the range 0 <= y[i, t] < V         [0,V)
        ...
        #     from the image features. This should produce an array of shape (N, H)#
        ############################################################################
        #print(features.shape,W_proj.shape,b_proj.shape)
        ...
        #     array of shape (N, T, V).                                            #
        ############################################################################
        scores, cache2 = temporal_affine_forward(h, W_vocab, b_vocab)
        ...
        ############################################################################
        # backward pass
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dscores, cache2)
        if self.cell_type == 'rnn':    
    ...
    def sample(self, features, max_length=30):
        ...
        captions[:,0] = np.array([self._start for i in range(N)])
        if self.cell_type == 'lstm':
            c = np.zeros(h.shape)
        for i in range(1,max_length):
            x , _ = word_embedding_forward(captions[:,i-1],W_embed)
            if self.cell_type == 'rnn':
